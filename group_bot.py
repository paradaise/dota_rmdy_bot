...
async def handle_award_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE, award_type: str
):
...
async def handle_fun_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE, cmd_type: str
):
    if not context.args or not context.args[0].startswith("@"):
        await update.message.reply_text(
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🎮 <i>Всего игр:</i> <b>{lobby['total_games']}</b>\n"
        f"🏆 <i>Побед:</i> <b>{lobby['wins']}</b>\n"
        f"💀 <i>Поражений:</i> <b>{lobby['losses']}</b>\n"
        f"📈 <i>Винрейт:</i> <b>{lobby['winrate']:.1f}%</b>\n"
    )

    if mvp:
        text += "\n⭐ <b>Топ MVP:</b>\n" + "\n".join(
            f"▫️ {name}: <b>{count}</b>" for name, count in mvp
        )

    if lvp:
        text += "\n\n💀 <b>Топ LVP:</b>\n" + "\n".join(
            f"▫️ {name}: <b>{count}</b>" for name, count in lvp
        )
...
async def contacts_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        contacts_text, parse_mode="HTML", disable_web_page_preview=True
    )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "🐌 Замедлить голос", callback_data="slow_voice"
                    )
                ],
                [InlineKeyboardButton("📝 Замедлить текст", callback_data="slow_text")],
                [InlineKeyboardButton("🔙 Назад", callback_data="back")],
            ]
            await query.edit_message_text(
                "🐌 <b>Режим замедления</b>\n\nВыберите действие:",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode="HTML",
            )
        elif query.data == "slow_voice":
            await query.edit_message_text(
                "🎤 Отправьте голосовое сообщение или аудиофайл, и я замедлю его!",
                parse_mode="HTML",
            )
        elif query.data == "slow_text":
            await query.edit_message_text(
                "📝 Введите текст после команды <code>/slow_text</code>, и я превращу его в замедленную речь.",
                parse_mode="HTML",
            )
